...
const opendexDatabasePath = 'opendex';
...
describe('Backup', () => {
  ...
  beforeAll(async () => {
    await fs.promises.writeFile(opendexDatabasePath, backups.xud.startup);

    await backup.start({
      backupdir,
      loglevel: 'error',
      dbpath: opendexDatabasePath,
    });
  });
  ...
  test('should write XUD database backups on startup', () => {
    expect(fs.readFileSync(path.join(backupdir, 'opendex'), 'utf8')).toEqual(backups.xud.startup);
  });

  test('should detect XUD database backups on new event', async () => {
    fs.writeFileSync(opendexDatabasePath, backups.xud.event);

    // Wait to make sure the file watcher handled the new file
    await new Promise((resolve, reject) => {
      setTimeout(reject, 3000);
      backup.on('changeDetected', (path) => {
        if (path.endsWith(opendexDatabasePath)) {
          resolve();
        }
      });
    });
  ...
  afterAll(async () => {
    ...
    await fs.promises.unlink(opendexDatabasePath);
  });
});
