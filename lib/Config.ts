...
class Config {
  ...
  constructor() {
    ...
    switch (platform) {
      case 'win32': {
        ...
        this.opendexdir = path.join(homeDir, 'Xud');
  ...
  public load = async (args?: { [argName: string]: any }): Promise<boolean> => {
    ...
    const configPath = path.join(this.opendexdir, 'xud.conf');
  ...
  private getDefaultDbPath = () => {
    return path.join(this.opendexdir, `xud-${this.network}.db`);
  };

  private getDefaultLogPath = (): string => {
    return path.resolve(this.opendexdir, 'logs', 'xud.log');
  };
