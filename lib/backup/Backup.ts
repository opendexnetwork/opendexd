...
class Backup extends EventEmitter {
  ...
  private opendexBackupTimer = setInterval(() => {
    if (this.databaseChangedMap.get('opendex') === true) {
      const backupPath = this.getBackupPath('opendex');
      const content = this.readDatabase(this.config.dbpath);
      this.writeBackup(backupPath, content);
      this.databaseChangedMap.set('opendex', false);
    }
  }, 180000);
  ...
  public start = async (args: { [argName: string]: any }) => {
    ...
    // Start the opendex database filewatcher
    this.startFilewatcher('opendex', this.config.dbpath).catch(this.logger.error);
  ...
  public stop = () => {
    ...
    clearInterval(this.opendexBackupTimer);
  };
